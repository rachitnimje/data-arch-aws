...
export default function AdminApplicationsPage() {
  ...
  const handleDownloadResume = useCallback(
    async (id: number, resumeUrl: string | undefined) => {
      if (!resumeUrl) {
        toast({
      ...
      try {
        const res = await fetch(
          `/api/resume-url?key=${encodeURIComponent(resumeUrl)}`
        );
                      ...
                      <tr key={app.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4">
                          <select
                            ...
                            className={`px-2 py-1 rounded text-sm border ${getStatusClass(
                              app.status
                            )}`}
                        ...
                        <td className="py-3 px-4 text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleDownloadResume(app.id, app.resume_url)
                              }
                              disabled={
                                !app.resume_url || isDownloading[app.id]
                              }
